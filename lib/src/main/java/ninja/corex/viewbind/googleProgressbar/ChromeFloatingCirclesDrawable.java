...
public class ChromeFloatingCirclesDrawable extends Drawable implements android.graphics.drawable.Drawable.Callback {
	...
	private enum ProgressStates {
		GREEN_TOP, YELLOW_TOP, RED_TOP, BLUE_TOP
	}
	...
	protected boolean onLevelChange(int level) {
		...
		if (level < CENT_LEVEL / acceleration) { // go
			if (level < MID_LEVEL / acceleration) {
				...
				colorSign |= 2;
				// from mid to end
				offsetPercentage = (max_speed * temp_level - 0.5 * acceleration * temp_level * temp_level) / distance
						+ 1.0;
				offset = (int) (offsetPercentage * ef_width / 2); // x and y direction offset
			}
		} else { // back
