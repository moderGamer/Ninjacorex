...
import static ninja.corex.RenderUi.IElasticityState.*;
...
import ninja.corex.RenderUi.adapters.RecyclerViewElasticityAdapter;
...
public abstract class ElasticityBounceEffectBase implements IElasticity, View.OnTouchListener {
  ...
  protected IElasticityStateListener mStateListener =
      new ElasticityListenerStubs.ElasticityStateListenerStub();
  protected IElasticityUpdateListener mUpdateListener =
      new ElasticityListenerStubs.ElasticityUpdateListenerStub();

  /**
   * When in over-scroll mode, keep track of dragging velocity to provide a smooth slow-down for the
   * bounce-back effect.
   */
  protected float mVelocity;

  /**
   * Motion attributes: keeps data describing current motion event. <br>
   * Orientation agnostic: subclasses provide either horizontal or vertical initialization of the
   * agnostic attributes.
   */
  ...
  protected interface IDecoratorState {
     ...
     * specified in {@link IElasticityState}.
     *
   ...
   * initiated (i.e. when scrolled further when the view is at one of its displayable ends). <br>
  ...
  protected class IdleState implements IDecoratorState {
    ...
    public void handleEntryTransition(IDecoratorState fromState) {
      mStateListener.onOverScrollStateChange(
          ElasticityBounceEffectBase.this, fromState.getStateId(), this.getStateId());
    }
  }
   ...
   * <p><br>
   * <br>
   * The state is exited - thus completing over-scroll handling, in one of two cases: <br>
   * When user lets go of the view, it transitions control to the bounce-back state. <br>
   * When user moves the view back onto a potential 'under-scroll' state, it abruptly transitions
   * control to the idle-state, so as to return touch-events management to the normal
   * over-scroll-less environment (thus preventing under-scrolling and potentially regaining regular
   * scrolling).
   */
  protected class OverScrollingState implements IDecoratorState {
    ...
    public boolean handleMoveTouchEvent(MotionEvent event) {
      ...
      float deltaOffset =
          mMoveAttr.mDeltaOffset
              / (mMoveAttr.mDir == mOverScrollStartAttr.mDir
                  ? mTouchDragRatioFwd
                  : mTouchDragRatioBck);
      ...
      if ((mOverScrollStartAttr.mDir
              && !mMoveAttr.mDir
              && (newOffset <= mOverScrollStartAttr.mAbsOffset))
          || (!mOverScrollStartAttr.mDir
              && mMoveAttr.mDir
              && (newOffset >= mOverScrollStartAttr.mAbsOffset))) {
        translateViewAndEvent(
            view, mOverScrollStartAttr.mDir, mOverScrollStartAttr.mAbsOffset, event);
      ...
      if (dt
          > 0) { // Sometimes (though rarely) dt==0 cause originally timing is in nanos, but is
                 // presented in millis.
      ...
      mUpdateListener.onOverScrollUpdate(
          ElasticityBounceEffectBase.this, mCurrDragState, newOffset);
  ...
  /**
   * When entered, starts the bounce-back animation. <br>
   * Upon animation completion, transitions control onto the idle state; Does so by registering
   * itself as an animation listener. <br>
   * In the meantime, blocks (intercepts) all touch events.
   */
  protected class BounceBackState
      implements IDecoratorState, Animator.AnimatorListener, ValueAnimator.AnimatorUpdateListener {
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
      translateView(
          mViewAdapter.getView(), mOverScrollStartAttr.mDir, (Float) animation.getAnimatedValue());
      mUpdateListener.onOverScrollUpdate(
          ElasticityBounceEffectBase.this, STATE_BOUNCE_BACK, (Float) animation.getAnimatedValue());
    }
    ...
    protected Animator createAnimator() {
      ...
      // Exception: If wasn't dragging in 'forward' direction (or velocity=0 -- i.e. not dragging at
      // all),
      // skip slow-down anim directly to the bounce-back.
      if (mVelocity == 0f
          || (mVelocity < 0 && mOverScrollStartAttr.mDir)
          || (mVelocity > 0 && !mOverScrollStartAttr.mDir)) {
      ...
      slowdownDuration =
          (slowdownDuration < 0 ? 0 : slowdownDuration); // Happens in counter-direction dragging
      ...
      ValueAnimator slowdownAnim =
          createSlowdownAnimator(
              (int) slowdownDuration, mAnimAttributes.mAbsOffset, slowdownEndOffset);

      // Set up the bounce back animation, bringing the view back into the original, pre-overscroll
    ...
    protected ValueAnimator createSlowdownAnimator(
        int slowdownDuration, float slowdownStartOffset, float slowdownEndOffset) {
  ...
  public ElasticityBounceEffectBase(
      IElasticityAdapter viewAdapter,
  ...
  public void setOverScrollStateListener(IElasticityStateListener listener) {
    mStateListener =
        (listener != null ? listener : new ElasticityListenerStubs.ElasticityStateListenerStub());
  }

  @Override
  public void setOverScrollUpdateListener(IElasticityUpdateListener listener) {
    mUpdateListener =
        (listener != null ? listener : new ElasticityListenerStubs.ElasticityUpdateListenerStub());
  }
  ...
  protected abstract void translateViewAndEvent(
      View view, boolean dir, float offset, MotionEvent event);
